...
| N-10 | Security        | No session ID or subject appears in any log line, span attribute or metric label. A response whose content **depends on** the session is not stored by any shared cache — this library's own or one downstream. A response that merely *touched* the session is not covered; see N-18. |
